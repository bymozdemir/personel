...
// özelBildirimGoster, ozelPromptGoster, ozelOnayGoster, bilgiNotunuGuncelleVeGoster)
...
function kategoriAdiGecerliMi(ad) {
  if (ad.includes('_')) {
    özelBildirimGoster('Kategori/alan adlarında alt çizgi (_) karakteri kullanılamaz. Lütfen farklı bir ad girin.');
    return false;
  }
  return true;
}
...
function grupTimYapisiniKaydet() {
  try {
    ...
    özelBildirimGoster("Grup/Tim yapısı kaydedilemedi! Hafıza dolu olabilir.");
  }
}
...
function grupEkle() {
  ...
  if (!val) return özelBildirimGoster("Lütfen bir grup adı yazın.");
  if (grupTimYapisi[val]) return özelBildirimGoster("Bu isimde bir grup zaten var.");
  ...
  özelBildirimGoster(`"${val}" grubu eklendi.`);
}
...
function timEkle() {
  ...
  if (!grup) return özelBildirimGoster("Önce bir grup seçin veya ekleyin.");
  if (!val) return özelBildirimGoster("Lütfen bir tim adı yazın.");
  if (!Array.isArray(grupTimYapisi[grup])) grupTimYapisi[grup] = [];
  if (grupTimYapisi[grup].includes(val)) return özelBildirimGoster("Bu isimde bir tim zaten bu grupta var.");
  ...
  grupTimListesiniCiz();
  bilgiNotunuGuncelleVeGoster();
  özelBildirimGoster(`"${val}" timi "${grup}" grubuna eklendi.`);
}
...
function grupDuzenle(eskiAd) {
  ozelPromptGoster("Yeni grup adını girin:", eskiAd, (girilenDeger) => {
    ...
    let yeniAd = girilenDeger.trim();
    if (yeniAd === "" || yeniAd === eskiAd) return;
    if (grupTimYapisi[yeniAd]) return özelBildirimGoster("Bu isimde bir grup zaten var.");
    ...
    özelBildirimGoster("Grup adı başarıyla güncellendi.");
  });
}
...
function timDuzenle(grup, eskiAd) {
  ozelPromptGoster("Yeni tim adını girin:", eskiAd, (girilenDeger) => {
    ...
    if (grupTimYapisi[grup].includes(yeniAd)) return özelBildirimGoster("Bu isimde bir tim zaten bu grupta var.");
    ...
    bilgiNotunuGuncelleVeGoster();
    personelListesiniCiz();
    özelBildirimGoster("Tim adı başarıyla güncellendi.");
  });
}
...
function personeleGrupTimAta() {
  ...
  if (ataSeciliPersonelIds.size === 0) return özelBildirimGoster("Lütfen en az bir personel seçin.");
  if (!grup || !tim) return özelBildirimGoster("Lütfen grup ve tim seçin.");
  ...
  grupTimListesiniCiz();
  bilgiNotunuGuncelleVeGoster();
  özelBildirimGoster(`${sayac} personel "${grup} - ${tim}" timine atandı.`);
}
...
function personelAtamasiniKaldir() {
  ...
  grupTimListesiniCiz();
  bilgiNotunuGuncelleVeGoster();
  özelBildirimGoster(`${sayac} personelin grup/tim ataması kaldırıldı.`);
}
...
function pinBelirle() {
  ozelPromptGoster("4-6 haneli yeni PIN girin:", "", (girilen) => {
    ...
    if (!/^\d{4,6}$/.test(pin)) return özelBildirimGoster("PIN sadece rakamlardan oluşmalı ve 4-6 haneli olmalı.");
    ozelPromptGoster("PIN'i onaylamak için tekrar girin:", "", (girilen2) => {
      if (girilen2 === null) return;
      if (girilen2.trim() !== pin) return özelBildirimGoster("Girdiğiniz PIN'ler eşleşmedi. Lütfen tekrar deneyin.");
      localStorage.setItem('guvenlikPin', basitHashUret(pin));
      pinDurumunuGoster();
      özelBildirimGoster("PIN başarıyla ayarlandı.");
    });
  });
}
...
function pinKaldir() {
  ozelPromptGoster("PIN korumasını kaldırmak için mevcut PIN'i girin:", "", (girilen) => {
    ...
    if (basitHashUret(girilen.trim()) !== localStorage.getItem('guvenlikPin')) {
      return özelBildirimGoster("PIN yanlış.");
    }
    localStorage.removeItem('guvenlikPin');
    pinDurumunuGoster();
    özelBildirimGoster("PIN koruması kaldırıldı.");
  });
}
...
function pinIleKorunanIslemiCalistir(islemFn) {
  ...
  ozelPromptGoster("Bu işlem için PIN girin:", "", (girilen) => {
    if (girilen === null) return;
    if (basitHashUret((girilen || "").trim()) !== localStorage.getItem('guvenlikPin')) {
      return özelBildirimGoster("PIN yanlış. İşlem iptal edildi.");
    }
    islemFn();
  });
}
...
function tumVerileriSifirla() {
  pinIleKorunanIslemiCalistir(() => {
    ozelOnayGoster("Tüm verileriniz, şablonlarınız ve ayarlarınız kalıcı olarak silinecek. Bu işlemi onaylıyor musunuz?", (onaylandi) => {
      ...
      if (typeof özelBildirimGoster === 'function') {
        özelBildirimGoster("Tüm veriler ve ayarlar sıfırlandı, sayfa yenileniyor...");
      }
