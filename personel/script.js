...
// Bir personelin, arama kutusundaki metne (isim, sicil no veya kayıtlı herhangi bir
// alan içeriği) ve aktif grup/tim filtresine uyup uymadığını döndürür. personelAra()
// ile AYNI eşleşme mantığını kullanır; bu sayede "Hepsini Seç" ve "N seçili" sayacı,
// arama kutusu isim dışında bir alanla (sicil no veya form verisi) eşleşme
// gösterdiğinde de ekrandaki listeyle TUTARLI kalır. q önceden küçük harfe
// çevrilmiş ve boşlukları kırpılmış olmalıdır.
      ...
      if (Array.isArray(kayitlar)) {
        for (let kayit of kayitlar) {
          for (let alan in kayit) {
            if (String(kayit[alan] || '').toLowerCase().includes(q)) return true;
          }
        }
      }
    }
...
function personelAra() {
  ...
  filtrelenmis.slice(0, anaListeGosterilenSayisi).forEach(p => {
    ...
    if (q.length > 0 && !(p.adSoyad || "").toLowerCase().includes(q) && !String(p.id).toLowerCase().includes(q)) {
      // Not: Önceden iç "for (alan in kayit)" döngüsündeki break sadece o döngüyü
      // durduruyordu; dıştaki "for (kayit of kayitlar)" döngüsü devam ettiği için
      // birden fazla kayıt varsa etiket SON bulunan eşleşmeyle değişebiliyordu.
      // dislari.some(...) kullanılarak ilk eşleşme bulunduğunda tüm arama tamamen durdurulur.
      Object.keys(p.veriler || {}).some(mainKey => {
        let kayitlar = p.veriler[mainKey];
        if (!Array.isArray(kayitlar)) return false;
        return kayitlar.some(kayit => {
          return Object.keys(kayit).some(alan => {
            let val = String(kayit[alan] || '');
            if (val.toLowerCase().includes(q)) {
              eslesmeEtiketi = `<span class="arama-eslesme-etiketi" style="font-size:11px; color:#1e88e5; background:#e3f2fd; padding:2px 6px; border-radius:4px; margin-left:8px;">${guvenliMetin(alan)}: ${guvenliMetin(val)}</span>`;
              return true;
            }
            return false;
          });
